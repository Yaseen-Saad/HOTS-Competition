document.addEventListener('DOMContentLoaded', () => {
  ...
  let memberCount = 1; // Start with one member (team leader)
  
  if (addMemberBtn) {
    addMemberBtn.addEventListener('click', addNewMember);
    addMemberBtn.addEventListener('keydown', (e) => {
      if (e.key === 'Enter' || e.key === ' ') {
        e.preventDefault();
        addNewMember();
      }
  ...
  function addNewMember() {
    if (memberCount < 4) {
      ...
      memberEl.className = 'member fade-in';
      memberEl.innerHTML = `
        <div class="member-header">
          <h3 class="member-title">Team Member ${memberCount}</h3>
          <button type="button" class="remove-member" data-member="${memberCount}" aria-label="Remove team member ${memberCount}">
            <i class="fas fa-times"></i>
          </button>
        </div>
        <div class="form-group">
          <label for="member${memberCount}-name">Full Name *</label>
          <input type="text" id="member${memberCount}-name" name="member${memberCount}-name" class="form-control" required>
        </div>
        <div class="form-group">
          <label for="member${memberCount}-email">Email *</label>
          <input type="email" id="member${memberCount}-email" name="member${memberCount}-email" class="form-control" required>
        </div>
        <div class="form-group">
          <label for="member${memberCount}-school">School *</label>
          <input type="text" id="member${memberCount}-school" name="member${memberCount}-school" class="form-control" required>
        </div>
      `;
      
      teamMembers.appendChild(memberEl);
      
      // Add focus to the new name field for accessibility
      setTimeout(() => {
        document.getElementById(`member${memberCount}-name`).focus();
      }, 100);
      
      // Add remove event listener
      const removeBtn = memberEl.querySelector('.remove-member');
  ...
  const handleRemoveMember = (e) => {
    const memberToRemove = e.target.closest('.member');
    
    // Add fade out animation
    memberToRemove.classList.remove('active');
    memberToRemove.style.opacity = '0';
    memberToRemove.style.transform = 'translateY(20px)';
    
    // Remove after animation completes
    setTimeout(() => {
      memberToRemove.remove();
      
      // Re-enable add button if hidden
      if (addMemberBtn.style.display === 'none') {
      ...
      // Update remaining member titles
      const remainingMembers = document.querySelectorAll('.member');
      remainingMembers.forEach((member, index) => {
        const title = member.querySelector('.member-title');
        if (index === 0) {
          title.textContent = 'Team Leader';
        } else {
          title.textContent = `Team Member ${index + 1}`;
        }
      });
      
      // Set focus to the add button for accessibility
      addMemberBtn.focus();
    }, 300);
  };
  ...
  if (registrationForm) {
    registrationForm.addEventListener('submit', (e) => {
      ...
      if (!language) {
        showError('language-group', 'Please select a language preference');
        hasError = true;
      }
      ...
      members.forEach((member, index) => {
        ...
        const name = document.getElementById(`member${memberNum}-name`).value;
        const email = document.getElementById(`member${memberNum}-email`).value;
        const school = document.getElementById(`member${memberNum}-school`).value;
        
        if (!name.trim()) {
          showError(`member${memberNum}-name`, 'Name is required');
          hasError = true;
        }
        
        if (!email.trim()) {
          showError(`member${memberNum}-email`, 'Email is required');
          hasError = true;
        } else if (!isValidEmail(email)) {
          showError(`member${memberNum}-email`, 'Please enter a valid email address');
          hasError = true;
        }
        
        if (!school.trim()) {
          showError(`member${memberNum}-school`, 'School is required');
          hasError = true;
        }
        ...
        if (index === 0) {
          ...
          if (!phone.trim()) {
            showError('leader-phone', 'Phone number is required');
            hasError = true;
          }
        }
      });
